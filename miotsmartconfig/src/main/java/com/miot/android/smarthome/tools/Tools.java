...
public class Tools implements IReceiver {
     ...
     *
     * @param map www.51miaomiao.com 杭州服务器 112.124.115.125 1887204221
     *            192.168.10.107
     *            <p/>
     *            192.168.10.200
     * @return
    ...
    public static boolean MiotFirstConfig(Map<String, String> map) {
        ...
        testString(makeFirst);
        return Tools.sendUdp(Tools.cuUdpSocket, "255.255.255.255",
                Tools.Search_Pu_Port, 1, makeFirst);
    }
    ...
    public void onReceive(int localPort, String host, int port, byte[] src,
        ...
        Map<String, String> map = respBaseAck.getResultMap();
        ...
        if (localPort == Search_Cu_Port) {
            ...
            if (respBaseAck != null && respBaseAck instanceof RespSmartConnectedAck) {
                m.what = FC_FIRST_CONFIG;
                m.obj = map;
                if (fcfirstConfig != null)
                    fcfirstConfig.sendMessage(m);
            } else if (respBaseAck != null && respBaseAck instanceof RespFc_completeAck) {
